...
public abstract class BlockContainerFalling extends BlockContainer {
    public static boolean fallInstantly;

    /* MODIFIED */
    public BlockContainerFalling(Material materialIn) {
        super(materialIn);
    }
        ...
        if ((worldIn.isAirBlock(pos.down()) || canFallThrough(worldIn.getBlockState(pos.down()))) && pos.getY() >= 0) {
            ...
            if (!fallInstantly && worldIn.isAreaLoaded(pos.add(-32, -32, -32), pos.add(32, 32, 32))) {
                if (!worldIn.isRemote) {
                    ...
                    EntityFallingSafe entityFallingSafe = new EntityFallingSafe(worldIn, pos, blockState);
                    onBeforeFall(worldIn, pos);
                    worldIn.spawnEntity(entityFallingSafe);
                }
            } else {
                onBeforeFall(worldIn, pos);
